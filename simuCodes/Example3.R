...
for(i in 11: N){
  ...
  datList <- gendata_simu_multi2(seed=i, nvec=nvec, a_interval = AI, p=p, q=q, d= d, 
                                 rank0=rank0,rho=rho, sigma2_eps=1)
  XcList <- datList$Xlist; 
  aList <- datList$aList
  ZList <- datList$Zlist; 
  res <- MultiCOAP(XcList, ZList, aList=aList, q=q, qs=qs,rank_use = rank0, verbose=T, init='MSFRVI')
  #str(res)
  ...
  ## COAP
  res_coap <- COAP_run(XcList, ZList, q=q,rank_use = rank0, aList=aList,verbose=F)
  # str(res_coap)
